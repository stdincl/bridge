...
class SocketIO {
	public static function send($action= "message",  $data = null, $address = "/socket.io/?EIO=2", $transport = 'websocket'){
		$socket = new SocketIO();
		return $socket->sendApi(
            IO_SOCKET_IO_SERVER, 
			IO_SOCKET_IO_SERVER_PORT, 
			$action,  
			$data, 
			$address, 
			$transport
    ...
    public function sendApi($host = null, $port = null, $action= "message",  $data = null, $address = "/socket.io/?EIO=2", $transport = 'websocket')
    {
